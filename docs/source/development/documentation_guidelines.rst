...
- The ``Host`` header ought to reflect the actual hostname of the Traffic Ops server - which should be "trafficops.ciab" for the CDN in a Box environment. This can be polluted when requests are made to a remotely running CDN in a Box on a different server.
